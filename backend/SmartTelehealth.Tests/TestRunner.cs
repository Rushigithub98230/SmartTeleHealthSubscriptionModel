...
public class TestRunner
{
    ...
    public TestRunner()
    {
        ...
        var host = CreateTestHost(configuration);
    ...
    /// <summary>
    /// Runs all billing service tests
    /// </summary>
    public async Task RunBillingServiceTestsAsync()
    {
        _logger.LogInformation("Starting Billing Service Tests...");

        try
        {
            var testClass = new BillingServiceTests();
            var testMethods = typeof(BillingServiceTests).GetMethods()
                .Where(m => m.GetCustomAttributes(typeof(FactAttribute), false).Length > 0)
                .ToList();

            foreach (var testMethod in testMethods)
            {
                _logger.LogInformation("Running test: {TestMethod}", testMethod.Name);
                
                try
                {
                    var task = (Task)testMethod.Invoke(testClass, null);
                    await task;
                    _logger.LogInformation("✅ Test passed: {TestMethod}", testMethod.Name);
                }
                catch (Exception ex)
                {
                    _logger.LogError(ex, "❌ Test failed: {TestMethod}", testMethod.Name);
                }
            }
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error running billing service tests");
        }
    }

    /// <summary>
    /// Runs all subscription service tests
    /// </summary>
    public async Task RunSubscriptionServiceTestsAsync()
    {
        _logger.LogInformation("Starting Subscription Service Tests...");

        try
        {
            var testClass = new SubscriptionServiceTests();
            var testMethods = typeof(SubscriptionServiceTests).GetMethods()
                .Where(m => m.GetCustomAttributes(typeof(FactAttribute), false).Length > 0)
                .ToList();
        ...
        {
            _logger.LogError(ex, "Error running subscription service tests");
        }
    }
    ...
    public async Task RunAllTestsAsync()
    {
        ...
        await RunBillingServiceTestsAsync();
        await RunSubscriptionServiceTestsAsync();
